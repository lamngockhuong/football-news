...
class BetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($keyword)) {
            $bets = $this->betRepository->search($keyword, config('repository.pagination.limit'));
            $bets->appends($request->only('q'));
        } else {
            $bets = $this->betRepository->bets(config('repository.pagination.limit'), [['id', 'desc']]);
        }

        return view('admin.bet.index', compact('bets', 'matches'));
    }
    ...
    public function edit($id)
    {
        try {
            ...
            return view('admin.bet.index', compact('bet', 'matches', 'bets'));
    ...
    public function update(BetRequest $request, $id)
    {
        try {
            ...
            return redirect()->route('bets.index')->with('notification', $notification);
        } catch (RepositoryException $e) {
            DB::rollback();
            $message = trans('admin.bet.index.edit.message.not_found');
            $notification = [
    ...
    public function destroy($id)
    {
        ...
        if (str_contains(url()->previous(), route('bets.edit', ['id' => $id]))) {
            return redirect()->route('bets.index')->with('notification', $notification);
        }
